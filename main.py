...
1. set param
2. load model, data
3. train model: compute output -> update weights
'''
...
print(device)
Herlev_TRAINSET_SIZE = 733        

parser = argparse.ArgumentParser(description='PyTorch Image Training')
parser.add_argument('-a', '--arch', metavar='ARCH', default='CNNTrans0', help='model structure')
...
def main_worker(ngpus_per_node, args):
    ...
    CNNTrans_build_func = get_CNNTrans_func_by_name(args.arch)   # build model
    model = CNNTrans_build_func(deploy=False)
    ...
    for epoch in range(args.start_epoch, args.epochs+1):
        ...
        print('\t')
        best_acc_train = train(train_loader, model, criterion, optimizer, epoch, args)
        torch.save(model.state_dict(), "./saveWeights/train.pth")  
        if global_acc_train < float(best_acc_train[0]):           
            print('==> Saving model..')
            global_acc_train = float(best_acc_train[0])
        if global_loss_train > float(best_acc_train[1]):           
            print('==> Saving model..')
            global_loss_train = float(best_acc_train[1])
        ...
        if global_acc < best_acc:
            print('==> Saving model..')
            global_acc = best_acc
            torch.save(model.state_dict(), './val.pth')
